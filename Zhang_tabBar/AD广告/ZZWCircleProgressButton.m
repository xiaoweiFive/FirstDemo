...
@interface ZZWCircleProgressButton()
...
@implementation ZZWCircleProgressButton
...
-(UIBezierPath *)bezierPath{
    if (!_bezierPath) {
        ...
        CGFloat heigh       = CGRectGetWidth(self.frame)/2;
        CGPoint centerPoint = CGPointMake(width, heigh);
        float   radius      = CGRectGetWidth(self.frame)/2;
        _bezierPath = [UIBezierPath bezierPathWithArcCenter:centerPoint radius:radius startAngle:degreesToRadians(-90) endAngle:degreesToRadians(270) clockwise:YES];
    }
    return  _bezierPath;
}
